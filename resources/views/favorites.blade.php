...
<body class="text-white">
    ...
    <main class="container mx-auto px-4 py-8">
        <div class="text-center mb-8">
            ...
            <p class="text-gray-400">People you're following and their tips</p>
        </div>

        @if($followedUsers->count() > 0)
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($followedUsers as $user)
                    <div class="bg-zinc-800 rounded-2xl p-6 hover:bg-zinc-700 transition-colors">
                        <div class="flex items-center gap-4 mb-6 pb-6 border-b border-zinc-700">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-gray-400 text-sm">{{ $user->followers()->count() }} followers</p>
                                @if($user->isTipster())
                        ...
                        <div class="text-center">
                            <a href="{{ route('favorites.user', $user) }}" class="w-full bg-emerald-400 hover:bg-emerald-500 text-black font-bold text-lg py-3 rounded-xl flex items-center justify-center gap-2 transition-colors">
                                View Their Tips
        ...
        @else
            <div class="text-center text-gray-400">
                <i class="fas fa-heart text-6xl mb-4 text-red-500"></i>
                <h2 class="text-2xl font-bold mb-2">No Favorites Yet</h2>
                <p class="mb-6">Start following tipsters to see their tips here.</p>
                <a href="/tips" class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors">
                    Browse Tips
                </a>
            </div>
        @endif
                ...
                .then(data => {
                    if(data.success) {
                        if(data.following) {
                            ...
                            // Remove this user card since they're no longer followed
                            this.closest('.bg-zinc-800').remove();
                        }
                    }
                })
                .catch(error => {
